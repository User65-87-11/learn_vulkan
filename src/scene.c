...
static void update_camera(struct Scene* scene, struct InputState * input,  float dt);

static void update_entities(struct Scene* scene, struct InputState * input,  float dt);

static void update_keys(struct Scene* scene, struct InputState * input, float dt);
...
void Scene_Init(struct Scene * scene, float aspect_ratio){
	...
	glm_perspective(
		glm_rad(scene->camera_data.fov),
		scene->camera_data.aspect_ratio, 
		scene->camera_data.near_plane,
		scene->camera_data.far_plane, 
		scene->camera_data.proj
	);
...
struct InstanceData * Scene_GeInstanceData(struct Scene * scene,  uint32_t position){
...
static void update_keys(struct Scene* scene, struct InputState * input, float dt){
  ...
  if (input->keys[GLFW_KEY_W] == GLFW_PRESS) {
    ...
    glm_vec3_muladds(scene->camera_data.front, cameraSpeed, scene->camera_data.pos);
  }

  if (input->keys[ GLFW_KEY_S] == GLFW_PRESS) {
    glm_vec3_mulsubs(scene->camera_data.front, cameraSpeed, scene->camera_data.pos);
  }
  ...
  if (input->keys[GLFW_KEY_SPACE] == GLFW_PRESS) {
    ...
    glm_vec3_muladds(scene->camera_data.up, cameraSpeed, scene->camera_data.pos);
  }
...
static void update_camera(struct Scene* scene, struct InputState * input,  float dt){
	...
	front[0] = cos(glm_rad( scene->camera_data.yaw)) * cos(glm_rad( scene->camera_data.pitch));
	front[1] = sin(glm_rad(scene->camera_data.pitch));
	front[2] = sin(glm_rad( scene->camera_data.yaw)) * cos(glm_rad( scene->camera_data.pitch));
	
	GLM_VEC3_COPY(scene->camera_data.front, front);
	
	vec3 cameraCenter;
	glm_vec3_add(scene->camera_data.pos, scene->camera_data.front, cameraCenter);
	glm_lookat(scene->camera_data.pos, cameraCenter, scene->camera_data.up, scene->camera_data.view);

	glm_mat4_mul( scene->camera_data.proj,scene->camera_data.view, scene->camera_data.view_proj);
}
