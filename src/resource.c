...
static void copyBufferOffset(VkBuffer dstBuffer,
	uint32_t dstOffset,
	VkBuffer srcBuffer,
	uint32_t srcOffset,
	VkDeviceSize size);

static void generateMipmaps(VkCommandBuffer command,
	VkImage* image,
...
static void copyBufferToImage(VkCommandBuffer command,
	VkBuffer* buffer,
	VkImage* image,
	uint32_t width,
	uint32_t height);
...
void Resource_CreateImage(uint32_t width,
	...
	vkGetImageMemoryRequirements2(
		device, &imageMemoryRequirementsInfo2, &memoryRequirements2);
...
void Resource_CreateBuffer(VkDeviceSize size,
	...
	VkResult res =
		vkCreateBuffer(device, &bufferCreateInfo, NULL, &out->handle);
...
void Resource_FreeBuffer(struct Buffer* out) {
	VkDevice device = Device_Get()->logical_device;
	if (out->handle) {
...
void Resource_mapBufferMemory(struct Buffer* buffer) {
	vkMapMemory(Device_Get()->logical_device, buffer->memory, 0, buffer->size,
		0, &buffer->mapped);
}
...
void Resource_CreateTexture(void* data,
	uint32_t width,
	uint32_t height,
	VkFormat format,
	...
	Resource_CreateImage(w, h, out->image.mip_levels, format,
		VK_IMAGE_TILING_OPTIMAL, IMAGE_TEXTURE_USAGE, IMAGE_TEXTURE_PROPS,
		&out->image);
	VkCommandBuffer command = Resource_beginSingleTimeCommands();

	Resource_transitionImageLayout(command, &out->image.handle,
	...
	copyBufferToImage(command, &staging.handle, &out->image.handle, w, h);
	generateMipmaps(
	...
	Resource_FreeBuffer(&staging);
	Resource_CreateImageView(&out->image, VK_IMAGE_ASPECT_COLOR_BIT);
