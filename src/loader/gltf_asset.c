...
static void parse_nodes(struct GLTFAsset* asset,cgltf_data* data);

struct GLTFAsset* gltf_AllocAsset() {

	struct GLTFAsset* ret = malloc(sizeof(struct GLTFAsset));
	memset(ret, 0, sizeof(struct GLTFAsset));

	return ret;
}
static void free_primitive_data(struct  GLTFPrimitive*  primitive){
     PRINT_FNAME;
/*
    ...
    */
	 ...
	 */
	 ...
	 if (primitive->texcoords) {
		 free(primitive->texcoords);
	 }
	 if (primitive->tangents) {
		 free(primitive->tangents);
	 }
...
void gltf_FreeAsset(struct GLTFAsset* asset){
	...
	if (asset) {
		if (asset->images) {
			for (int i = 0; i < asset->image_count; i++) {
				struct GLTFImage* ref = &asset->images[i];
				free(ref->pixels);
			}
			free(asset->images);
		}
		...
		if (asset->meshes) {
			for (int i = 0; i < asset->mesh_count; i++) {
				struct GLTFMesh* ref = &asset->meshes[i];

				if (ref->primitives) {

					for (int j = 0; j < ref->primitive_count; j++) {
						struct GLTFPrimitive* prim = &ref->primitives[j];
						free_primitive_data(prim);
					}
					free(ref->primitives);
				}
			}
...
static void print_gltf(char* path) {
	...
	for (int i = 0; i < data->meshes_count; i++) {
		...
		for (int j = 0; j < data->meshes[i].primitives_count; j++) {
			...
			for (int k = 0; k < data->meshes[i].primitives[j].attributes_count;
				 k++) {
				printf("  Attributes :");
				printf("   .name:%s\n",
...
static void parse_nodes(struct GLTFAsset* asset,cgltf_data* data){
    asset->nodes = malloc(data->nodes_count* sizeof(struct GLTFNode));
    memset(asset->nodes,UNSET_BYTE, data->nodes_count* sizeof(struct GLTFNode));
    
    asset->node_count = data->nodes_count;
    for(int i=0;i<asset->node_count;i++)
    {
        ...
        node_dst->child_count = node_src->children_count;
        node_dst->children = calloc(node_dst->child_count , sizeof(uint32_t));
		...
		for (cgltf_size j = 0; j < node_src->children_count; ++j) {
			...
			node_dst->children[j] = child_index;
		}
	}
}
static void parse_scene(struct GLTFScene* dst, cgltf_scene * src )
{
    dst->root_nodes = malloc(src->nodes_count* sizeof(struct GLTFNode));
    memset(dst->root_nodes,UNSET_BYTE, src->nodes_count* sizeof(struct GLTFNode));
    
    dst->root_node_count = src->nodes_count;

    for(int i=0;i<src->nodes_count;i++)
    {
...
void gltf_Load(
	...
	asset->scenes = calloc(data->scenes_count , sizeof(struct GLTFScene));
	asset->scene_count = data->scenes_count;

	asset->nodes = calloc(data->nodes_count , sizeof(struct GLTFNode));
	asset->node_count = data->nodes_count;

	asset->images = calloc(data->images_count , sizeof(struct GLTFImage));
	asset->image_count = data->images_count;

	asset->materials = calloc(data->materials_count, sizeof(struct GLTFMaterial));
	asset->material_count = data->materials_count;

	asset->meshes = calloc(data->meshes_count , sizeof(struct GLTFMesh));
	asset->mesh_count = data->meshes_count;

	for (int i = 0; i < data->scenes_count; i++) {
		parse_scene(&asset->scenes[i], &data->scenes[i]);
	}
...
void loadModel(char* path, uint32_t* indicesNum, uint32_t** indices,
	...
	for (int i = 0; i < primitive->attributes_count; i++) {
		...
		if (attr.type == cgltf_attribute_type_position) {
			...
			uint32_t stride =
				accessor->stride ? accessor->stride : sizeof(float) * 3;
	...
	for (int i = 0; i < primitive->attributes_count; i++) {
		...
		if (attr.type == cgltf_attribute_type_normal) {
			...
			uint8_t* base =
				(uint8_t*)view->buffer->data + view->offset + accessor->offset;
			uint32_t stride =
				accessor->stride ? accessor->stride : sizeof(float) * 2;
